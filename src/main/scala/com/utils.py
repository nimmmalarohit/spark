...
async def search_wiki_knowledge(tracker: Tracker) -> Tuple[str, Optional[str]]:
    ...
    try:
        ...
        # Process content to extract clean, complete information
        content_parts = content.split("\n\n", 1)

        # Complete content including any potential heading
        if len(content_parts) > 1:
            actual_content = content_parts[1]
        else:
            actual_content = content

        # Ensure we don't remove relevant content
        # Don't remove the # for headings as we want to keep them as part of the text
        # But do remove any extra whitespace
...
# Add this to initialize the knowledge base when the module is loaded
def init():
    """Initialize the knowledge base when the module is loaded."""
    try:
        if not is_knowledge_base_current():
            logger.info("Initializing knowledge base on module load")
            initialize_knowledge_base()
    except Exception as e:
        logger.error(f"Error initializing knowledge base on module load: {e}")
